...
class UserService:
    ...
    async def register(self, user_data: UserRegister) -> User:
        ...
        async with self.uow as uow:
            existing_email = await uow.user.get_user_by_email(user_data.email)
            if existing_email:
                raise UserAlreadyExist('Email уже зарегистрирован')
            if user_data.phone:
                existing_phone = await uow.user.get_user_by_phone(user_data.phone)
                if existing_phone:
            ...
            user = await uow.user.create_user(user_data=UserRegister(**user_dict))
        return user
    ...
    async def login(self, user_log: UserLogin) -> Tuple[Any, Any]:
        ...
        """
        async with self.uow as uow:
            existing_user = await uow.user.get_user_by_email(user_log.email)
    ...
    async def get_user_profile(self, access_token: str = Cookie(...)) -> User:
        ...
        async with self.uow as uow:
            user = await uow.user.get_user_by_id(int(payload['sub']))
        if not user:
    ...
    async def update_user(self, user_id: int, user_data: UserUpdate) -> User:
        ...
        async with self.uow as uow:
            ...
            updated_user = await uow.user.update(user, user_data)
    ...
    async def refresh_access_token(self, refresh_token: str) -> str:
        ...
        async with self.uow as uow:
            user = await uow.user.get_user_by_id(int(user_id))
        if not user:
